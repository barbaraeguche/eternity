...
class TranscendentalButton extends JButton implements ActionListener {
	 ...
	 * @param calc   the Calculator instance with which this button will be
	 *               associated.
	 */
	...
	// put all transcendental function implementation here **PLEASE SHOW YOUR
	// IMPLEMENTATION WITH YOUR REVIEWER BEFORE MAKING A PULL REQUEST**
	/**
	 * this method processes an action event triggered by a user interface
	 * component.
	 * 
		...
		try {
			...
			case "log_b(x)":
				if (parameterList.length > 2)
					...
					result = calculator.functions.logXBase10(parameterList[0]);
				else
			...
			calculator.displayLabel.setText(err.getMessage());
		} catch (TooManyArgumentsException exc) {
			calculator.displayLabel.setText("Error: Too Many Arguments");
		} catch (Exception err) {
			calculator.displayLabel.setText("An error occurred: " + err.getMessage());
		}
	}
}
